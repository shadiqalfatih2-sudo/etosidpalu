...
function About() {
  return <section className={`${styles.about} etos-about`} id="tentang"><div className={styles.sectionLabel}><span />Tentang Etos ID Palu</div><div className={styles.aboutGrid}><h2>Menumbuhkan pemimpin muda yang tangguh, berakar pada nilai, dan hadir membawa dampak.</h2><div className={styles.aboutCopy}><p>Etos ID Palu menghadirkan pembinaan yang tidak berhenti pada capaian akademik. Prosesnya dirancang sebagai ekosistem tumbuh: mengasah nalar, memperkuat spiritualitas, membangun kepemimpinan, dan menghubungkan gagasan dengan kebutuhan masyarakat.</p><p>Di sini, awardee belajar untuk tidak hanya menjadi penerima manfaat, tetapi juga menjadi pribadi yang mampu mengubah pengalaman menjadi kontribusi nyata.</p><Link href="/program">Lihat ekosistem pembinaan <span>→</span></Link></div></div></section>;
}

function Programs({ programs }: { programs: NativeProgram[] }) {
  return <section className={`${styles.programSection} etos-program-section`} id="program"><div className={styles.sectionHeadWide}><div><div className={styles.sectionLabel}><span />Program Etos ID Palu</div><h2>Program yang dirancang untuk membentuk manusia, bukan sekadar memenuhi agenda.</h2></div><p>Setiap program menghubungkan penguatan karakter, spiritualitas, kepemimpinan, kolaborasi, dan pengabdian.</p></div><div className={`${styles.programGrid} etos-home-program-grid`}>{programs.slice(0, 8).map((program, index) => <Link className={`${styles.programCard} etos-home-program-card`} href={`/program/${encodeURIComponent(program.id)}`} key={program.id}><div className={`${styles.programImage} etos-home-program-image`}>{program.preview ? <img src={program.preview} alt={program.name} /> : null}<div className={styles.programShade} /><div className={styles.programIndex}>{String(index + 1).padStart(2, '0')}</div><div className={`${styles.programOverlayContent} etos-home-program-copy`}><small>{program.category}</small><h3>{program.name}</h3><p>{program.summary}</p></div></div></Link>)}</div><div className={styles.sectionFootNote}><Link href="/program">Lihat seluruh program aktif →</Link></div></section>;
}

function Awardees({ awardees }: { awardees: NativeAwardee[] }) {
  return <section className={`${styles.awardeeSection} etos-awardee-section`} id="awardee"><div className={styles.awardeeHead}><div><div className={styles.sectionLabel}><span />Awardee Etos ID Palu</div><h2>Orang-orang yang bertumbuh dan membawa gagasan menjadi dampak.</h2></div><Link href="/awardee" className={styles.darkPill}>Lihat Semua Awardee</Link></div><div className={`${styles.awardeeGrid} etos-home-awardee-grid`}>{awardees.slice(0, 5).map((awardee) => <Link className={`${styles.awardeeCard} etos-home-awardee-card`} href={`/awardee/${encodeURIComponent(awardee.id)}`} key={awardee.id}><div className={`${styles.imageWrap} etos-home-awardee-image`}>{awardee.photo ? <img src={awardee.photo} alt={awardee.name} style={{ objectPosition: awardee.photoPosition }} /> : null}</div><div className={styles.cardBody}><small>{awardee.cohort ? `Angkatan ${awardee.cohort}` : 'Awardee Etos ID'}</small><h3>{awardee.name}</h3><p>{awardee.studyProgram}{awardee.university ? ` • ${awardee.university}` : ''}</p></div></Link>)}</div></section>;
}
...
function Publications({ publications }: { publications: NativePublication[] }) {
  ...
  return <section className={`${styles.publicationSection} etos-publication-section`} id="publikasi"><div className={styles.sectionHeadSimple}><div><div className={styles.sectionLabel}><span />Berita & Opini</div><h2>Catatan perjalanan, gagasan, dan dampak dari ekosistem Etos ID Palu.</h2></div><Link href="/#publikasi">Lihat Selengkapnya</Link></div>{lead ? <div className={`${styles.publicationLayout} etos-home-publication-layout`}><Link className={`${styles.leadPublication} etos-lead-publication`} href={publicationHref(lead)}><div className={`${styles.leadImage} etos-lead-publication-image`}>{lead.thumbnail ? <img src={lead.thumbnail} alt="" /> : null}</div><div className={`${styles.leadBody} etos-lead-publication-copy`}><div className={styles.meta}>{lead.kind} <span>•</span> {formatDate(lead.publishedAt)}</div><h3>{lead.title}</h3><p>{lead.excerpt}</p><span className={styles.readMore}>Baca selengkapnya →</span></div></Link><div className={styles.publicationList}>{rest.map((item, index) => <Link href={publicationHref(item)} className={`${styles.publicationRow} etos-publication-row`} key={`${item.kind}-${item.id}`}><div className={styles.publicationRowNumber}>{String(index + 2).padStart(2, '0')}</div><div className={styles.publicationRowText}><small>{item.kind} • {formatDate(item.publishedAt)}</small><h3>{item.title}</h3></div><div className={`${styles.publicationRowImage} etos-publication-row-image`}>{item.thumbnail ? <img src={item.thumbnail} alt="" /> : null}</div></Link>)}</div></div> : null}</section>;
}

function ClosingCta() {
  return <section className={`${styles.ctaSection} etos-cta-section`}><div><span>SUARA DARI EKOSISTEM ETOS</span><h2>Punya cerita, gagasan, atau pengalaman yang layak dibagikan?</h2></div><div className={styles.ctaActions}><Link href="/kirim-tulisan" className={styles.ctaPrimary}>Kirim Tulisan</Link><a href="https://www.instagram.com/etosidpalu/" target="_blank" rel="noreferrer" className={styles.ctaSecondary}>Instagram Etos ID Palu</a></div></section>;
}

function Footer() {
  return <footer className={`${styles.footer} etos-footer`}><div className={styles.footerBrand}><EtosLogo /><p>We Are Resilient Leader</p></div><div className={styles.footerLinks}><Link href="/#tentang">Tentang</Link><Link href="/program">Program</Link><Link href="/awardee">Awardee</Link><Link href="/#publikasi">Berita & Opini</Link></div><div className={styles.footerMeta}>© {new Date().getFullYear()} Etos ID Palu.</div></footer>;
}
...
export function NativeHomePreview({ heroes, programs, awardees, publications, stats }: { heroes: NativeHero[]; programs: NativeProgram[]; awardees: NativeAwardee[]; publications: NativePublication[]; stats: NativeHomeStats }) {
  return <main className={`${styles.page} native-home`}><SiteHeader /><HeroSlider heroes={heroes} stats={stats} /><About /><Programs programs={programs} /><Awardees awardees={awardees} /><Publications publications={publications} /><ClosingCta /><Footer /></main>;
}
